...
def get_sekolah(kec_code, jenjang):
    ...
    df_sekolah.to_csv(f'data/data_sekolah_kita/{jenjang}/{i}_detail_sekolah_kita_{kec_code.strip()}.csv', index=False)

df_wilayah = pd.read_csv('data/data_sekolah_kita/df_wilayah/wilayah_sekolah_kita.csv', dtype='str')

for i, wilayah in df_wilayah.iloc[62:len(df_wilayah)].iterrows():
    akreditasi = get_sekolah(wilayah.kec_code, 'dikdas')
