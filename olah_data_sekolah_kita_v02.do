...
import delimited "C:\Users\aaffa\OneDrive\Study Group Big Data\Data\sekolah_kita_v02.csv", clear
...
gen beda = kecamatankota_ln!=kec_name if dup_npsn == 1 // deteksi sekolah dengan npsn sama tapi berbeda kec_name dan kecamatankota_ln
drop if beda==1

*masih ada beberapa dengan duplikat npsn
duplicates tag npsn, gen(dup_npsn2)
gen tanggal_sk_date = date(tanggal_sk_pendirian,"DMY")

*drop berdasarkan SK pendirian yang lebih muda
egen lebihtua = min(tanggal_sk_date), by(npsn)
drop if tanggal_sk_date!=lebihtua & dup_npsn2==1

*drop berdasarkan yang tidak upload SK
duplicates tag npsn, gen(dup_npsn3)
drop if file_sk_operasional=="Silakan Upload SK" & dup_npsn3==1

* drop berdasarkan tanggal upload yang lebih baru
duplicates tag npsn, gen(dup_npsn4)
split tanggal_upload_sk_op, p(" ")
gen tanggal_upload_sk_op1date = date(tanggal_upload_sk_op1,"YMD")
egen uploadsk = max(tanggal_upload_sk_op1date), by(npsn)
drop if tanggal_upload_sk_op1date!=uploadsk & dup_npsn4==1

drop dup_npsn-uploadsk // hapus var yang tidak perlu
...
* ada univ luar negeri yang masuk di Kec. Kanggime
drop if inrange(bentuk_pendidikan,39,43) & kec_name == "kanggime"

* Hapus univ luar negeri dengan nama university (sudah dicek semuanya univ LN)
gen has_university = strpos(nama, "University") > 0
drop if has_university
drop has_university
...
notes : Data ini diambil tanggal 25 Mei - 23 Juni 2023
...
save "C:\Users\aaffa\OneDrive\Study Group Big Data\Data\sekolah_kita_v02.dta", replace